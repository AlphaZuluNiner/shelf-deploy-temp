  ...
  "node_modules/@mozilla/readability/Readability.js"(exports, module) {
    ...
    Readability2.prototype = {
      ...
      **/
       ...
       * @return Boolean
       */
      _hasAncestorTag: function(node, tagName19, maxDepth, filterFn) {
        maxDepth = maxDepth || 3;
        tagName19 = tagName19.toUpperCase();
        var depth = 0;
        while (node.parentNode) {
          if (maxDepth > 0 && depth > maxDepth)
            return false;
          if (node.parentNode.tagName === tagName19 && (!filterFn || filterFn(node.parentNode)))
            return true;
          node = node.parentNode;
          depth++;
        }
...
var loopSegment = ({ [NEXT]: next, [END]: end }, json2) => {
  ...
  const last = json2.length - 1;
  const value = json2[last];
  if (typeof value === "number" && value < 0)
    json2[last] += NODE_END;
  else
    json2.push(NODE_END);
};
var attrAsJSON = (attr, json2) => {
  json2.push(ATTRIBUTE_NODE, attr.name);
  const value = attr[VALUE].trim();
  if (value)
    json2.push(value);
};
var characterDataAsJSON = (node, json2) => {
  const value = node[VALUE];
  if (value.trim())
    json2.push(node.nodeType, value);
};
var nonElementAsJSON = (node, json2) => {
  json2.push(node.nodeType);
  loopSegment(node, json2);
};
var documentTypeAsJSON = ({ name, publicId, systemId }, json2) => {
  json2.push(DOCUMENT_TYPE_NODE, name);
  if (publicId)
    json2.push(publicId);
  if (systemId)
    json2.push(systemId);
};
var elementAsJSON = (element, json2) => {
  json2.push(ELEMENT_NODE, element.localName);
  loopSegment(element, json2);
};
...
var CharacterData = class extends Node2 {
  ...
  toJSON() {
    const json2 = [];
    characterDataAsJSON(this, json2);
    return json2;
  }
};
...
async function extractArticle(url) {
  const response = await fetch(url, {
    headers: {
      "User-Agent": "Mozilla/5.0 (compatible; Shelf/1.0; +https://shelf.app)",
  ...
  if (!article) {
    ...
    return {
      title,
      content: ogDesc ? `<div><p>${escapeHtml(ogDesc)}</p><p><a href="${escapeHtml(url)}">View original</a></p></div>` : `<div><p>Could not extract content from this page.</p><p><a href="${escapeHtml(url)}">View original</a></p></div>`,
      textContent: ogDesc || "",
      author: null,
      excerpt: ogDesc || null,
      siteName: null,
      wordCount: ogDesc ? ogDesc.split(/\s+/).length : 0,
      estimatedReadTime: 1,
      imageUrl: null,
      publishedAt: null,
      originalHtml: html,
  ...
  const textContent2 = article.textContent || "";
  const wordCount = textContent2.split(/\s+/).filter(Boolean).length;
  const estimatedReadTime = Math.max(1, Math.ceil(wordCount / 238));
  const excerpt = textContent2.substring(0, 200).trim();
  const ogImage = document.querySelector('meta[property="og:image"]')?.getAttribute("content");
  const twitterImage = document.querySelector('meta[name="twitter:image"]')?.getAttribute("content");
  const imageUrl = ogImage || twitterImage || null;
  const pubDate = document.querySelector('meta[property="article:published_time"]')?.getAttribute("content") || document.querySelector("time[datetime]")?.getAttribute("datetime") || null;
  return {
    title: article.title,
...
async function extractTweet(url) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const { document } = parseHTML(embedHtml);
      const textContent2 = document.body?.textContent?.trim() || "";
      if (textContent2.length > 0) {
        const wordCount = textContent2.split(/\s+/).filter(Boolean).length;
        const styledContent = `
          <div class="tweet-content">
  ...
  return {
    ...
    content: `
      <div class="tweet-content">
        <p>This tweet could not be extracted. It may be from a private account or behind a login wall.</p>
        <p style="margin-top: 16px;">
          <a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">View on X &#x2197;</a>
        </p>
      </div>
    `,
    textContent: "Tweet could not be extracted.",
    author: null,
    excerpt: "Tweet could not be extracted.",
    siteName: "Twitter/X",
    wordCount: 0,
    estimatedReadTime: 1,
    imageUrl: null,
...
var handleArticles = {
  ...
  async create(request, env) {
    ...
    if (!url && !content) {
      return json({ error: "Provide either url or content" }, 400);
    }
    ...
    if (url) {
      ...
      if (sourceType === "twitter") {
        articleData = await extractTweet(url);
      } else {
        articleData = await extractArticle(url);
      }
      articleData.url = url;
      articleData.sourceType = sourceType;
    } else {
      articleData = {
        url: null,
        ...
        estimatedReadTime: Math.max(1, Math.ceil(content.split(/\s+/).filter(Boolean).length / 238)),
        imageUrl: null,
        publishedAt: null,
        originalHtml: null,
        extractionFailed: false
    ...
    let contentKey = null;
    let originalHtmlKey = null;
    if (articleData.content) {
      contentKey = `articles/${id}/content.html`;
      await env.STORAGE.put(contentKey, articleData.content);
    }
    if (articleData.originalHtml) {
      originalHtmlKey = `articles/${id}/original.html`;
      await env.STORAGE.put(originalHtmlKey, articleData.originalHtml);
    }
    await env.DB.prepare(`
      INSERT INTO articles (
        id, url, title, author, excerpt, source_type, status,
        content_key, original_html_key, word_count, estimated_read_time,
        site_name, image_url, published_at, saved_at,
    ...
    `).bind(
      ...
      articleData.author,
      articleData.excerpt,
      articleData.sourceType || "article",
      contentKey,
      originalHtmlKey,
      articleData.wordCount || null,
      articleData.estimatedReadTime || null,
  ...
  async list(request, env) {
    ...
    let query2 = "SELECT * FROM articles WHERE 1=1";
    const params = [];
    if (status) {
      query2 += " AND status = ?";
      params.push(status);
    ...
    if (sourceType) {
      query2 += " AND source_type = ?";
      params.push(sourceType);
    }
    if (favorite === "1") {
      query2 += " AND is_favorite = 1";
    }
    query2 += " ORDER BY saved_at DESC LIMIT ? OFFSET ?";
    params.push(limit, offset);
    const { results } = await env.DB.prepare(query2).bind(...params).all();
    ...
    return json({ articles: results, total: countResult.total, limit, offset });
  }
};
var handleArticleById = {
  /**
  ...
  async get(id, env) {
    ...
    if (article.content_key) {
      const obj = await env.STORAGE.get(article.content_key);
      if (obj)
        content = await obj.text();
    }
    let originalHtml = null;
    if (article.original_html_key) {
      const obj = await env.STORAGE.get(article.original_html_key);
      if (obj)
        originalHtml = await obj.text();
    }
    return json({ ...article, content, original_html: originalHtml });
  },
  /**
   * PATCH /api/articles/:id — update article fields
   */
  async update(id, request, env) {
    const article = await env.DB.prepare("SELECT * FROM articles WHERE id = ?").bind(id).first();
    if (!article)
      return json({ error: "Article not found" }, 404);
    const body = await request.json();
    ...
    for (const field of allowedFields) {
      if (body[field] !== void 0) {
        updates.push(`${field} = ?`);
        values.push(field === "tags" ? JSON.stringify(body[field]) : body[field]);
      }
    }
    if (updates.length === 0) {
  ...
  /**
   * DELETE /api/articles/:id — delete article + R2 files
   */
  async remove(id, env) {
    const article = await env.DB.prepare("SELECT * FROM articles WHERE id = ?").bind(id).first();
    if (!article)
      return json({ error: "Article not found" }, 404);
    const keysToDelete = [article.content_key, article.original_html_key, article.pdf_key, article.epub_key].filter(Boolean);
    await Promise.all(keysToDelete.map((key2) => env.STORAGE.delete(key2)));
...
async function handleFileUpload(request, env) {
  ...
  if (ext === "pdf" || file.type === "application/pdf") {
    ...
    return json({ error: "Unsupported file type. Only PDF and EPUB are accepted." }, 400);
  }
  const id = crypto.randomUUID();
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const fileKey = `articles/${id}/file.${ext}`;
  ...
  const estimatedWords = sourceType === "pdf" ? Math.round(fileSizeKB * 0.15) : Math.round(fileSizeKB * 0.2);
  const estimatedReadTime = Math.max(1, Math.ceil(estimatedWords / 238));
  const pdfKey = sourceType === "pdf" ? fileKey : null;
  const epubKey = sourceType === "epub" ? fileKey : null;
  await env.DB.prepare(`
    INSERT INTO articles (
      id, url, title, author, excerpt, source_type, status,
      content_key, original_html_key, pdf_key, epub_key,
      word_count, estimated_read_time,
      site_name, image_url, published_at, saved_at,
      tags, created_at, updated_at
    ) VALUES (?, NULL, ?, NULL, ?, ?, 'inbox', NULL, NULL, ?, ?, ?, ?, NULL, NULL, NULL, ?, '[]', ?, ?)
  `).bind(
    id,
    title,
    `${sourceType.toUpperCase()} file \xB7 ${(fileSizeKB / 1024).toFixed(1)} MB`,
    sourceType,
    pdfKey,
    epubKey,
    estimatedWords,
...
async function handleArticleFile(id, env) {
  const article = await env.DB.prepare("SELECT * FROM articles WHERE id = ?").bind(id).first();
  if (!article)
    return json({ error: "Article not found" }, 404);
  const fileKey = article.pdf_key || article.epub_key;
  if (!fileKey)
    return json({ error: "No file associated with this article" }, 404);
  const obj = await env.STORAGE.get(fileKey);
  if (!obj)
    return json({ error: "File not found in storage" }, 404);
  const mimeType = article.source_type === "pdf" ? "application/pdf" : "application/epub+zip";
...
async function handleArticlePdf(id, env) {
  return json({ error: "PDF export not yet implemented." }, 501);
}
...
var src_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/api/articles/upload" && request.method === "POST") {
        return handleFileUpload(request, env);
      }
      if (path === "/api/articles" && request.method === "POST") {
        return handleArticles.create(request, env);
      }
      if (path === "/api/articles" && request.method === "GET") {
        return handleArticles.list(request, env);
      }
      const fileMatch = path.match(/^\/api\/articles\/([^/]+)\/file$/);
      if (fileMatch && request.method === "GET") {
        return handleArticleFile(fileMatch[1], env);
      }
      const articleMatch = path.match(/^\/api\/articles\/([^/]+)$/);
      if (articleMatch) {
        const id = articleMatch[1];
        if (request.method === "GET")
          return handleArticleById.get(id, env);
        if (request.method === "PATCH")
          return handleArticleById.update(id, request, env);
        if (request.method === "DELETE")
          return handleArticleById.remove(id, env);
      }
      const pdfMatch = path.match(/^\/api\/articles\/([^/]+)\/pdf$/);
      if (pdfMatch && request.method === "POST") {
        return handleArticlePdf(pdfMatch[1], env);
      }
      return serveFrontend(path, env);
    } catch (err) {
...
async function serveFrontend(path, env) {
  const MIME_TYPES = {
    ".html": "text/html",
  ...
  let key2 = path === "/" ? "_site/index.html" : `_site${path}`;
  let object = await env.FILES.get(key2);
  if (!object && !path.includes(".")) {
    key2 = "_site/index.html";
    object = await env.FILES.get(key2);
  }
  if (!object) {
    return new Response("Not Found", { status: 404 });
  }
  const ext = key2.substring(key2.lastIndexOf("."));
